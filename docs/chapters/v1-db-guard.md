...
如果你跑一遍 `bash scripts/v1-burst.sh 1001 300 60`，再看 `snapshot` 里 `oversoldUnits` 和 `duplicateUserCount` 都是 `0`，就可以把实验结果和上面的实现一一对应起来：前者来自乐观锁条件扣减，后者来自唯一索引最终裁决。
...
这个脚本会自动完成重置、并发请求和快照输出。如果你想再复查一次，也可以手动查快照：
...
当然，v1 不是终点。它把正确性立住了，但热点库存仍在数据库，冲突高时重试会放大 DB 压力，吞吐上限仍然受制于数据库写能力。所以接下来的 v2 才会顺理成章地登场：把热点库存前置到 Redis，用 Lua 脚本把校验和扣减收敛成原子步骤，在保持正确性的同时继续提升抗压能力。
